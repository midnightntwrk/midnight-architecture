...
Sparse Pedersen commitment is an homomorphic commitment scheme used in Note Commitments and Nullifiers to derive the unique ρ value for a Sapling note. It is also used in the Spend statement to confirm use of the correct ρ value as an input to nullifier derivation. 
a commitment construction that hashes an asset type
descriptor to the bases of an extended Pedersen commitment
such that the resulting commitments are additively
homomorphic, which facilitates proving the balZswap
4
ancing of amounts.
...
\subsection{State transitions and Oracles}

Kachina contracts divide their state into a shared public state and a local private state for each party. 
We define the state $\phi$ by the tuple $(\sigma,\rho)$ where $\sigma$ is the public state and $\rho$ consists of $\rho_p$ for each party $p$.
Transition functions are applied over the pair $(\sigma,\rho)$ where each party can only change their local state $\rho_p$.\\
The contract does not update the state directly but has access to public and private oracles that operate on the state transition.

The contract makes queries to these oracles by calling functions which update the
state, and return information about it. Lets call $PrivQueryF$ the function that queries the private oracle and $PubQueryF$ the one that queries the public oracle.
$PrivQueryF$ queries the oracles used for key generation $PRQGenKey$, honest transaction generation and recording those transactions into the ledger. \\
Queries made by users are used to prove the interaction with the public state is correct by giving correct responses to queries they prove they know the input which will make the series of queries.
To prove the interaction with the public state correct, users capture the queries they made, and the responses they
