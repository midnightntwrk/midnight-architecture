...
Encryption secret key is an element of the embedded curve's (JubJub) scalar field, generated by iteratively hashing seed with domain separator "midnight:esk", then truncated to specified number of bytes. Resulting byte sequence is interpreted to scalar assuming a little-endian layout and taken modulo the field prime. In naive pseudocode (simplifying for readability):
```ts
...
function encryptionSecretKey(seed: Buffer): BigInt {
    ...
    const sampledBytes = sampleBytes(JubJubScalar.bytes + 8, "midnight:esk", seed);
