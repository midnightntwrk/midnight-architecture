...
- a field type,
- an embedded elliptic curve group, and
- hashing operations.
...
It would be unnecessary effort to port ZKIR intrinsics to TypeScript (or every other
...
another.  Such deviation would be potentially catastrophic, as it would invalidate
produced transactions, breaking liveness.
...
but we depend on TypeScript to run TFs, however has the drawback of maintaining
an additional compiler back-end.
