...
- once the target epoch to trigger `system.setCode` is reached - the first block producer to produce a block in the epoch includes that extrinsic with proper WASM code

Parameters for various environments could be set as follows:

- devnet and testnet: 75% blocks in an epoch are needed to trigger update
- mainnet: 90% of blocks
...
2. There is a certain code centralization - the runtime, with consensus code being involved. Theoretically it can be avoided by leveraging `implementation_name` in Substrate or proposing a change to Substrate to keep track of multiple runtime alternatives.
3. Nodes still need a restart to update in many cases - namely those, where native code is updated.
4. Team needs to be careful with runtime interface upgrades to not introduce changes making it impossible to validate chain from the genesis. For example by performing regularly a check whether newer version can validate current chain of a testnet/mainnet from the genesis.
