...
These are all encoded in straightforward way, with `u32` being encoded as little-endian.
...
| `add`    |   `12` | `-{a, b}      +{c}`        |                               - |         `1` | sets `[c] := [a] + [b]` |
| `sub`    |   `13` | `-{a, b}      +{c}`        |                               - |         `1` | sets `[c] := [b] - [a]` |
...
Warning: This fundamentally changed, to not return a `Maybe`, but instead fail
on `head` of empty list, as otherwise we need to perform a potentially
