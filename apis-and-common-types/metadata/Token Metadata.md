...
These token types are calculated as a hash of contract address and contract-defined data (called domain separator). Single contract can mint multiple types of tokens, but they will all be related to that contract. There are no ledger-imposed restrictions in sending and receiving existing tokens.
...
|------------------|----------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| type             | ✅        | literal "token"               | constant indicating that rest of the data follows this definition                                                                                                                                     |
| subject          | ✅        | bech32 / hex-string / bytes   | ledger-serialized token type, it is the default identifier of token and its metadata                                                                                                                  |
...
| image            | ❌        | string                        | an URI (allowing a data base64 one) of an image representing the token, it's also very common, but many existing tokens do not have the image attached                                                |
...
| supply           | ❌        | non-negative integer          | how many token units were minted / are allowed to be minted                                                                                                                                           |
| extensions       | ❌        | array of extensions           | additional types given token implement, described in [extenstions section below](#extensions)                                                                                                         |
...
The metadata listed above is the basic, core one, that is likely to be provided by all tokens and easily converted from tokens from other chains, when bridged. For that reason there exists "extensions" field, which includes a list of additional metadata implemented/defined for a given token, represented as object with two fields: `type` to indicate kind of extension and `data` for the data itself. Since bridging is a common predicted use-case for this - it is acknowledged that data present in extensions will likely be duplicated. 
...
No. It is already being observed with various token types, e.g. on Solana or Ethereum, that there are assets, which are not clearly fungible or non-fungible, and it is their usage/contract that tells about what they are (one example being a notion of partial ownership of a token, e.g. a real estate). For that reason it would make sense to not distinguish token's fungibility in any way in the basic metadata, but instead accommodate both cases in the common metadata and define clear extension points if very specific types arise in the future.

No indication of fungibility also makes it easier to accommodate varying shapes of tokens across ecosystems. 

### This API focuses on token metadata, should it accommodate for possibility of attaching metadata to other kinds of objects?

This API defines token metadata only. Though in a way, which will allow to define other metadata formats for different kinds of objects.
...
### There are technically possible cases, where contract decides to mint different token types to different users, depending on their input. If and how should they be accounted for?

Within the scope of this API, it would be perfectly fine to register new metadata according to some template whenever new token type is identified. This likely would need to be automated to be useful.

An alternative could be extension to this API to:
- indicate that subject is being computed
- provide an expression/function describing how is the domain separator computed

Because of amount of work (how to define the functions, how to index&query the data, etc.) it would require in order to be really useful, it should not be taken into consideration unless patterns/standards in how tokens are being implemented, indicate it is necessary.
