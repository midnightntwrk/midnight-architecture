...
- The phase :: The public state of an election has four distinct phases in which different operations are allowed: =REGISTERING=, =VOTING=, =COUNTING=, and =CALLED=b.  The contract tracks this phase in a public variable.
...
There is an easy solution to this problem, and it is included in our design but not our implementation.


** No Transparency of Voter Demographics

It's not possible for an outsider to audit the voter registration records on chain.

This could be addressed by placing this contract within a larger application in which some form of transparent registration was implemented on chain, or at least some proof of inclusiveness or one-vote-per-person.

** No Anti-Bribery Mechanism

Our contract makes it easy for a voter to prove to any observer of the chain that they voted in a particular way.  It would be easy to implement, say, a vote-buying contract that paid a reward for each unique proof of a vote with a desired value.
