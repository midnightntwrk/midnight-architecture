...
- Authorization nonce :: Kachina has no user accounts to use for authorization; instead, operations that require authorization are guarded by hashes.  In our contract, only Coordinators can mint REG tokens, so an authorization hash guards this operation.  It is the hash of a value known only to the Controller.  The ZK proof for the Mint transaction must include proof that the Controller knows a value whose hash is equal to the authorization hash.
...
* Spotlight on Zero Knowledge

The eVoting contract described here relies on zero-knowledge proofs to restrict public state changes based on privately-held facts from each participant.  Instead of disclosing the facts as justification of the correctness of a state update, the participants supply proofs that their updates are correct; they construct these proofs off chain using their private facts, yielding a proof in which those facts are irretrievable.  Nodes verify the proofs without seeing any of the actual facts that justify correctness.

** Plain-English Proofs Used in eVoting

The eVoting app constructs a new ZK proof for every transaction submitted to the blockchain.  Each distinct transaction type has its own ZK circuit, and that circuit is used off chain to construct a validity proof and again on chain to verify that proof.  It can be helpful to look at each transaction type in the eVoting example and describe in plain English what is being proven by the circuit.

- Mint a REG token :: This is a restricted operation that can be performed only by the Coordinator.  The coordinator must prove:
  - that it can knows a value whose hash is equal to the authorization hash (i.e., prove it is the Coordinator)
  - that the contract is in the REGISTERING state
- Cast a Vote :: The voter must prove:
  - that the input REG token //r// is a valid path in REG.Merk
  - that its nullifier is not in REG.nulls
  - that the voted option is a valid option for this election
- Count a Vote for Option //o// :: The voter must prove:
  - that the input VOT token //v// is a valid path in VOT.Merk
  - that its nullifier is not in VOT.nulls
  - that its voted option is equal to //o//
- Advance the Phase from //X// to //Y// :: The initiator must prove:
  - that the public "phase" is //X//
  - that the current consensus time is greater than or equal to the specified timeout =t_X=

The proofs concerning the REG and VOT tokens are done in the same manner as serial numbers / nullifiers in Zerocash:

- The eligible public key / vote commitment are pseudorandom projections of private data: The private key and the private key + vote respectively.
- We create an independant one-way projection of the same data, and publish that, checking that it does not already exist.
- Due to the pseudo-randomness of the projections, the two are unlinkable
- Due to the uniqueness of the underlying data and the projections, the projection cannot be revealed twice without being detected as a duplicate.


Kachina nodes verify transactions by first checking the given ZK proof against the current public state using a circuit specified in the contract deployment.  Only when this proof succeeds does the verifier apply the state updates to yield a modified public state.


** Linkability and the Need for a Private L1

The application we have described here could be implemented on any number of smart-contract platforms.  Although ZK proofs are central to the implementation, these kinds of proofs can be implemented without an L1 that specifically supports ZK and privacy.  The game Dark Forest, for example,  is built around ZK proofs but runs on an EVM.  The Espresso Systems "CARE" demo implements a zcash system on an EVM, using all the same data structures and similar ZK proofs as our eVoting dApp.  Why, then, do we argue that Kachina and our zcash-based ledger are both required to implement privacy?

The answer is linkability.  Ethereum and Cardano, like all non-private smart-contract platforms, leak information that allows linkability and, ultimately, de-anonymization.  Privacy is undermined by the very act of paying to submit a transaction.  Our zcash-based base currency ensures privacy in our payments layer.  Combined with the Lares contract ledger, which has no unshielded owners, this ensures end-to-end unlinkability for our end users.

If we implemented this same dApp on an Ethereum blockchain, for example, each voter's single user account would be billed for both their vote and their "count" disclosure, as well as any other activities performed by the same user account.  While their vote details would still be protected by ZK proofs, their participation in one or both rounds could potentially be linked to their other activities on the platform.  Midnight and the Kachina programming model combine to protect users from this kind of accidental linkability.
...
This implementation (at the time of this writing) does not implement all of the specification described in this document.  Some of the key differences:

- It does not have a fixed set of legal voting options.
- Only the Coordinator can advance the public state through its phases, and there are no timeouts to ensure the phases last for some pre-determined interval.
- Voters assume their transactions will succeed, so they don't track pending private-state updates.
