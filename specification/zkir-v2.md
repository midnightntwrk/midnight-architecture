...
**Rehearsal semantics:**
`<add a b, mem> -> mem ++ (mem[a] ++ mem[b])`

**Circuit semantics:**
`<add a b, mem> -> w := ADD(mem[a], mem[b]); mem ++ w
...
**Rehearsal semantics:**
`<assert cond, mem> -> mem, if mem[cond] = 0`
`<assert cond, mem> -> FAIL, otherwise

**Circuit semantics:**
`<assert cond, mem> -> w := NONZERO(mem[cond]); mem`
...
**Rehearsal semantics:**
`<cond_select bit a b, mem> -> mem ++ mem[b], if mem[bit] = 0`
`<cond_select bit a b, mem> -> mem ++ mem[a], if mem[bit] = 1`
`<cond_select bit a b, mem> -> FAIL, otherwise

**Circuit semantics:**
`<cond_select bit a b, mem> -> w0 := IS_ZERO(mem[bit]), w1 = SELECT(w0, mem[a], mem[b]); mem ++ w1
