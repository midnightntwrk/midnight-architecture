...
mutual 
  data Type (Δ : TypeContext) : Set where

    -- Compact Types 
    Boolean  : Type Δ
    UInteger[<=_] : (n : ℕ) → Type Δ
    UInteger[_]   : (n : ℕ) → Type Δ 
    Field         : Type Δ 
    Void          : Type Δ
    Bytes[_]      : (n : ℕ) → Type Δ 
    Vector[_,_]   : (n : ℕ) → (T : Type Δ) → Type Δ
    Opaque[_]     : (s : String) → Type Δ
    Enum          : enum ∈ Δ → Type Δ
    Struct        : struct n ∈ Δ → (Fin n → Type Δ) → Type Δ

    -- Ledger types 
    Counter            : Type Δ 
    Cell               : (Tⱽ : Type Δ) → CompactType Tⱽ → Type Δ
    SetT               : (Tⱽ : Type Δ) → Type Δ
    Map                : (Tᴷ Tⱽ : Type Δ) → Type Δ  
    ListT              : (Tⱽ : Type Δ) → Type Δ
    MerkleTree         : (d : ℕ) → (Tⱽ : Type Δ) → Type Δ
    HistoricMerkleTree : (d : ℕ) → (Tⱽ : Type Δ) → Type Δ  

    -- Type variables can be ledger types and compact types? 
    Var           : tvar ∈ Δ → Type Δ

  variable T₁ T₂ T₃ T T′ : Type Δ

  -- A predicate proving that `T` is a Compact type 
  data CompactType {Δ} : (T : Type Δ) → Set where
    ...
    isStruct    : ∀ (x : struct n ∈ _) args → CompactType (Struct x args) 

LedgerType : (T : Type Δ) → Set
LedgerType T = ¬ CompactType T

Context : TypeContext → Set
Context Δ = List (Type Δ)
