#+TITLE: Risk Management for Midnight Block Finality
#+AUTHOR: Jon Rossie
...
This document is concerned with a specific risk Midnight faces: loss
of liveness or safety for the consensus protocol, \grandpa, that
finalizes blocks.  Finality plays a critical role in Midnight because,
unlike most blockchain clients, Midnight's smart-contract client
~midnight.js~ is unable to process rollbacks.  When the client
observes a confirmed transaction, it applies pending local state
updates in a destructive manner.  Because of this (temporary)
limitation, Midnight has been designed to assume fast finalization to
reduce user-visible latencies while protecting against data loss.

Figure [[fig:arch]] shows a simplified set of Midnight components.
The Midnight Node observes its Cardano Node via DB Sync, and does so
at a delay of 12 hours because that is Cardano's longest-chain
finality window.  But most Midnight clients are more concerned with
the finality delay for transactions they post to the Midnight Node.
It relies on the Indexer to provide only finalized Midnight
transactions, to prevent possible data loss should rollback occur on
the Midnight Node.  Anything that disrupts the orderly finalization of
blocks, via the \grandpa{} consensus protocol, will impact clients.
If \grandpa{} experiences liveness issues that delay finalization for
unbounded periods, Midnight will appear offline to its clients even if
new blocks are being produced.  If \grandpa{} experiences a safety
violation that allows it to finalize contradictory forks, clients
...
The rest of this document presents both a detailed analysis of
Midnight's finality risks and an analysis of the effectiveness of
Ariadne in reducing this risk.  We end with recommended best practice
for dynamically adjusting Ariadne's parameters to maximize the
system's ability to tolerate faults that could impact finality.
...
candidate participants will be excluded from some committees.  This
is true even if the committee size $k$ is much larger than the number
$n$ of candidates.  Whenever a candidate with large stake wins an
additional seat, that seat is no longer available to other candidates.
The effect when stakes vary widely is to effectively exclude the
lowest-staked candidates from committees.  If we equate trust with 
...
Ariadne follows these steps to create a committee:
1. Reserve $F$ of the $k$ committee seats for the federated nodes.
2. Use weighted random selection to populate the $k-F$ seats in the
   usual way, choosing from the available participants and allowing
   the same participant to occupy multiple seats.  This yields a
   committee composition: \[V = [(P_1, v_1), ..., (p_{k-F}, v_{k-F})]\]
3. Dilute the voting strength of each committee member by
   $(1-\varphi)$, converting real stake to virtual stake.
   \[V' = [(P_1, v_1(1-\varphi)), ..., (p_{k-F},
   v_{k-F}(1-\varphi))]\]
4. Compute the virtual stake of the federated block as $\varphi S$.
5. Add $F$ federated members to the committee, each having $\varphi
   S/F$ voting strength.

The result is a committee whose net voting strength is still $S$, but
with the effective voting strength of all permissionless members
diluted to shift $\varphi S$ voting strength to the federated block.
...
we might have a $\varphi$ of 34%, but we may need to drop to 20%
shortly thereafter and we may even go to 0% if we are lucky enough to
have thousands of participating SPOs.
...
** Version History
- **\docversion{} (2025-02-11)**: Initial version.
