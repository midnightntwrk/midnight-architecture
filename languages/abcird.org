...
*** Execute Transition Function as a Program
**** Call Transition Function
**** Query Private Oracle
**** Query Public Oracle
**** Query Context Oracle

*** Collect Witness Vector During Transition Function Execution
**** Question:  Is this equivalent to collecting a transcript and then passing it through an encoder?

*** Encode Publlic Transcript as Statement
**** Run Encoder of Structured Data as Field Elements

*** Interpret/Compile Transition Function to a Proving Key
**** Run Snarkie Interpreter

*** Generate ZK Proof using Proving Key, Witness, and Statement
**** Call into the Underlying Proving System

** Verifying on Node

*** Encoding Public Transcript as Statement
**** Run Encoder of Structured Data as Field Elements

*** Interpret/Compile Transition Function to a Verifier Key
**** Run Snarkie Interpreter

*** Verify ZK Proof usign Verifier Key and Statement
**** Call into the Underlying Proving System

*** Verify Public Oracle Transcript
**** Query Public Oracle

** Transition Function Dependencies

*** Private Oracle
- private oracle state
- private oracle implementation, that matches the private oracle interface.
*** Public Oracle
- public oracle state,
- public oracle implementation, that matches the public oracle interface.

*** Contrext Oracle
- context oracle implementation, that matches the context oracle interface.

*** Transition Function Runtime

* Deploy Story
** TODO How will we deploy smart contracts?

* Upgradability Story
To fully understand this, we need to firstly discuss the execution model.

** Bytecode VM/Bytecode Interpreter
Jakub's assumption is that we have single component that is responsible for
- calling TF,
- proving, and
- verification.
...
** 2022-04-28 Abcird Weekly Meeting
*** Agenda:
**** Update [Kent]: Current state of abcirdc
**** Update [Anatolii]: Current state of ZKIR
**** Update [Thomas]: Witness generation discussions
***** WASM witness generation
*** Kent update:
**** Surface language is much the same
***** Modules have been added
***** As have type parameters & circuit overloading
***** Structs have become types
***** Type inference has been added
**** Converting to a form of SSA
Concern that it's not what we need
**** We also need to decide on our compilation model
Do =extern= circuits link to libraries, and where?
*** Anatolli Update:
**** JSON-based IR
**** Public and private input numbers, list of gates
***** Gates executed linearly
***** Gates can reference outputs of prior gates
*** Wasm Witness generation
**** In the Plonk VM we need callbacks for external witness generation (public/private oracle)
***** For the rust side we'll be exposing these as C FFI/taking C function pointers
***** Something more direct? https://demille.github.io/wasm-ffi/docs/rust/
**** Can we actually Plonk =StandardComposer= in WASM
***** Will it survive stress tests: Poseidon hash
***** Basic need: Compile to Plonk contraints, witness generation
***** Advanced: Compile to prover/verifier keys, proving/verifying
**** Javascript needs en/decoders for abcird/Plonk datastructures
**** Dmitry was able to compile zk-garage/plonk to wasm
***** He needs help to ensure the benchmarks are being run correctly
***** Will aim to get some preliminary measurements by next week
